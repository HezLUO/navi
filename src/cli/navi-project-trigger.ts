...
export async function inspectProjectTrigger(projectDir: string): Promise<ProjectTriggerState> {
  return (await inspectProjectTriggerDocument(projectDir)).state;
}

export interface ProjectTriggerDocumentInspection {
  state: ProjectTriggerState;
  text?: string;
}

export async function inspectProjectTriggerDocument(projectDir: string): Promise<ProjectTriggerDocumentInspection> {
