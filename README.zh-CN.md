...
这个 alpha 是 GitHub source package，面向愿意从仓库测试的 Codex 用户和开发者。公开 npm/marketplace/一键安装仍不在范围内。

请先按照 [package README](./plugins/along-working-thread/README.md) 完成显式 plugin 安装。然后在此仓库中运行：
...
`navi doctor` 用于 troubleshooting，不是日常步骤；它会检查 source-alpha 前提条件并指出缺失时的修复方向。
